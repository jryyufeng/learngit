...
TEMPRALATIONSHIP = 'temp_realtime_relationship'
...
class RealtimeRandomWeibo(object):
    ...
    def get_realtime_weibo_from_mongodb(self):
        ...
        return now_weibo_all[self.count1-25:self.count1]
    ...
    def get_all_weibo_from_mongodb(self):
    ...
    def get_all_threaten_weibo_from_mongodb(self):

        # TODO:把这个改成通过机器判别后获得的恶意用户关系
        weibo_table = self.db[REALTIMEWEIBOT]
        now_weibo_threaten = []
        for i in weibo_table.find():
            now_weibo_threaten.append(i)
        return now_weibo_threaten

    def get_temp_threaten_relationship_from_mongodb(self):
        weibo_table = self.db[TEMPRALATIONSHIP]
        now_weibo_threaten = []
        for i in weibo_table.find():
            now_weibo_threaten.append(i)
        return now_weibo_threaten

    def _get_target_province_weibo(self, province):
        weibo_table = self.db[REALTIMEWEIBO]
        weibo_list = []
        for weibo in weibo_table.find():
            if str(weibo['province']) == str(province):
                weibo_list.append(weibo)
        return weibo_list

    def get_target_province_weibo(self, a):
        if a == 1:
            province = "北京"
        elif a == 2:
            province = '上海'
            ...
            province = '湖南'
        return self._get_target_province_weibo(province)

    def _get_target_province_threaten_weibo(self, province):
        weibo_table = self.db[REALTIMEWEIBOT]
        weibo_list = []
        for weibo in weibo_table.find():
            if str(weibo['province']) == str(province):
    ...
    def get_target_province_threaten_weibo(self, a):
        if a == 1:
            province = "北京"
        elif a == 2:
            province = '上海'
            ...
            province = '湖南'
        return self._get_target_province_threaten_weibo(province)
